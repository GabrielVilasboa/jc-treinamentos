<template>
    <div class="main-infos flex flex-row justify-between items-center max-lg:max-w-full" :class="width">
        ...
        <div class="flex ">
            <img v-if="icon" :src="icon" class="h-5 mr-4 self-center" :class="iconFunction ? 'cursor-pointer': 'cursor-auto'" @click="iconFunction">
            <p class="name text-lg font-semibold cursor-pointer" @click="openItem">{{ props.title }}</p>
        </div>

        <div class="flex">
            <img v-if="extraButton"
            class="h-5 cursor-pointer transition-transform duration-300 mr-4"
            :src="extraButton"
            alt="extraFunction"
            @click="extraFunction()" />
            <img class="h-5 cursor-pointer transition-transform duration-300"
